...
| 008 | `008-void-approval-parity-p2n2.md` | Reverse port FROM B (owner order 2026-07-19): void-sale approval workflow — parity record, but flags a real adaptation your verbatim reversal logic doesn't need yet (payment-account-aware void reversal, relevant once you build digital payments) plus a new rank-based self-approval exemption not in your build |
| 009 | `009-vendors-ap-ledger-parity-t3.md` | Reverse port FROM B (owner order 2026-07-19): Vendors + Cost Schedule + AP Ledger — parity record; flags 3 app-layer adaptations (companyId source, explicit branch picker vs "first active branch," offline-queue wiring) and a real empty-chart-of-accounts bug caught only by E2E |
| 010 | `010-payroll-bonus-t3-3.md` | Original work (owner order 2026-07-19): optional bonus/incentive on Disburse Wage — nothing to port, a template for the "append a defaulted arg" evolution pattern if you extend your own payroll disbursement function |
| 011 | `011-payroll-attendance-leave-overtime-disbursement-payout-methods-p2pr1-5.md` | Original work (owner backlog, 2026-07-20): attendance/leave/overtime tracking + wage disbursement approval workflow + multiple payout methods (P2PR1–P2PR5) — two self-approval design patterns (beneficiary-check vs literal requester-check, pick by whether money moves), a `CREATE OR REPLACE FUNCTION` return-type gotcha, a Radix dialog-overflow bug, and an unfixed net=0 journal crash worth checking your own equivalent function for |
| 012 | `012-payroll-formatted-payslip-p2pr6.md` | Original work (owner backlog, 2026-07-20): formatted/printable payslip, the last of 6 payroll sub-items — zero migration/RPC, reuses existing FKs via PostgREST embeds instead of a new list RPC, with a deliberate graceful-degradation design (generic labels, not errors) when the viewer's RLS blocks the joined account/user row |
