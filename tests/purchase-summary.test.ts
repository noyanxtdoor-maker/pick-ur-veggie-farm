...
const recv = (item_id: string, total: number, qty: number, source_type: 'online' | 'physical', source_name: string, date: string): PurchaseReceiving =>
  ({id: `${item_id}-${date}`, company_id: 'c', branch_id: 'b', item_id, quantity: qty, total_amount: total, source_type, source_name, source_contact: null, vendor_id: null, bought_by: null, received_date: date, created_at: ''});
